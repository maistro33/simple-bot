"""Temu platform plugin — extracts product data from temu.com product pages."""
...
_DOMAIN_PATTERN = re.compile(r"(?:^|\.)temu\.com$", re.IGNORECASE)


class TemuPlugin(PlatformPlugin):
    """Handles Temu product URLs."""

    platform = PlatformName.TEMU

    def matches(self, url: str) -> bool:
        """Return True if the URL points at Temu."""
        return domain_matches(url, _DOMAIN_PATTERN)

    def fetch_product_data(self, url: str) -> ProductData:
        """Fetch and normalise Temu product data via OpenGraph tags, with AI fallback."""
        html = fetch_html(url)
        data = extract_open_graph_data(html)

        if not data.title or not data.price:
            logger.debug("Temu OpenGraph extraction incomplete; using AI fallback for {}", url)
