"""eBay platform plugin — extracts product data from ebay.* listing pages."""
...
_DOMAIN_PATTERN = re.compile(r"(?:^|\.)ebay\.(?:com|co\.uk|de|fr|it|es|ca|com\.au)$", re.IGNORECASE)


class EbayPlugin(PlatformPlugin):
    """Handles eBay listing URLs across regional domains."""

    platform = PlatformName.EBAY

    def matches(self, url: str) -> bool:
        """Return True if the URL points at any eBay regional domain."""
        return domain_matches(url, _DOMAIN_PATTERN)

    def fetch_product_data(self, url: str) -> ProductData:
        """Fetch and normalise eBay listing data via OpenGraph tags, with AI fallback."""
        html = fetch_html(url)
        data = extract_open_graph_data(html)

        if not data.title or not data.price:
            logger.debug("eBay OpenGraph extraction incomplete; using AI fallback for {}", url)
