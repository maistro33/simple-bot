"""Amazon platform plugin — extracts product data from amazon.* product pages."""
...
_DOMAIN_PATTERN = re.compile(r"(?:^|\.)amazon\.(?:com|co\.uk|de|fr|it|es|ca|com\.mx|com\.br|in|co\.jp)$", re.IGNORECASE)


class AmazonPlugin(PlatformPlugin):
    """Handles Amazon marketplace product URLs across regional domains."""

    platform = PlatformName.AMAZON

    def matches(self, url: str) -> bool:
        """Return True if the URL points at any Amazon regional domain."""
        return domain_matches(url, _DOMAIN_PATTERN)

    def fetch_product_data(self, url: str) -> ProductData:
        """Fetch and normalise Amazon product data via OpenGraph tags, with AI fallback."""
        html = fetch_html(url)
        data = extract_open_graph_data(html)

        if not data.title or not data.price:
            logger.debug("Amazon OpenGraph extraction incomplete; using AI fallback for {}", url)
